...
{
	int index = CTypedPtrArray<T>::AddTail(t);
	BubbleUp(index);
}
...
public:
	...
	{
		CTypedPtrDblElement < T >* pNewEle = new CTypedPtrDblElement < T >;//创建新节点
		pNewEle->Data(t);//设置新节点数据
		pNewEle->Prev(pEle);//将新节点的Prev指针设置为pEle节点（插入到pEle之后）
		pNewEle->Next(pEle->Next()); //将新节点的Next指针设置为原pEle节点的后一个节点
		pNewEle->Prev()->Next(pNewEle); //将新节点的前一个节点的后向指针Next设置为新节点
		pNewEle->Next()->Prev(pNewEle);//将新节点的后一个节点的前向指针Prev设置位新节点
		m_iCount++; //计数增加
		return pNewEle; //返回插入的节点地址
	}
